...
TEST(linear_allocator,
     create_with_nullptr_and_zero_memory_with_parametrized_constructor) {
...
TEST(linear_allocator,
     swap_empty_linear_allocator_and_allocator_with_valid_values) {
  std::size_t const size{16};
...
TEST(linear_allocator, create_allocator_using_move_constructor) {
  ...
  std::function<linear_allocator(std::size_t const, void const*)>
      test_function =
...
TEST_F(test_linear_allocator, clear_allocated_memory_using_reset_method) {
  allocator.reset();
...
TEST_F(test_linear_allocator, check_that_allocation_is_possible) {
  ...
  auto const is_allocation_possible =
      allocator.is_allocation_possible(allocation_size);
...
TEST_F(test_linear_allocator, allocate_bytes_of_memory) {
  ...
  void const* first_allocated_memory{
      allocator.allocate(first_allocation_size, alignment)};

  std::size_t const second_allocation_size{4};
  void const* second_allocated_memory{
      allocator.allocate(second_allocation_size, alignment)};

  ASSERT_THAT(allocator.get_address(), Eq(address.get()));
  ASSERT_THAT(allocator.get_size(), Eq(size));
  ASSERT_THAT(allocator.get_used_size(),
              Eq(first_allocation_size + second_allocation_size));
  ASSERT_THAT(first_allocated_memory, Eq(address.get()));
  ASSERT_THAT(second_allocated_memory,
              Eq(address.get() + first_allocation_size));
}

TEST_F(test_linear_allocator,
       release_all_occupied_memory_completely_inside_allocator) {
  allocator.clear();
...
TEST_F(test_linear_allocator, allocate_twelve_bytes_with_alignment_at_8) {
  ...
  void const* allocated_memory_ptr{
      allocator.allocate(allocation_size, allocation_alignment)};

  u8 const adjustment =
      allocation_alignment - (allocation_size & (allocation_alignment - 1));
